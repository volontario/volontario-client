(function() {
  function AppController(
    ...
    $scope.toggleColor = function(color) {
      ...
      $localStorage.filter.color =
        $localStorage.filter.color.length && $localStorage.filter.color.indexOf(color) >= 0 ?
          _.without($localStorage.filter.color, color) :
          $localStorage.filter.color.concat([color]);
    };
    ...
    */
    ...
    $scope.$watch(
      function() {
        return $sessionStorage.total.top || $sessionStorage.total.bottom;
      },
      function() {
        calcTotal();
      }
